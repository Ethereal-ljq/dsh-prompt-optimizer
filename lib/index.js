/**
 * @dsh-external/dsh-prompt-toolkit — host 半（P0 骨架）。
 *
...
export const name = '@dsh-external/dsh-prompt-toolkit'
...
function collectDialogueDigest(ctx, sessionId, turns, mode) {
  ...
  if (!sessionId) return { mode: m, turns: n, degraded: true, reason: 'no-session-id', userTurns: 0, items: [] }
  let session = null
  ...
  if (!session) return { turns: n, degraded: true, reason: 'session-not-live', userTurns: 0, items: [] }
  let events = []
  try { events = typeof session.ownEvents === 'function' ? session.ownEvents() : [] } catch { events = [] }
  const seq = []
  for (const ev of events) {
    if (!ev || !ev.type) continue
    if (ev.type === 'user/message') {
      const text = textOfMessage(ev.data)
      if (text) seq.push({ role: 'user', text: text.length > HISTORY_USER_MAX_CHARS ? text.slice(0, HISTORY_USER_MAX_CHARS) + '…（截断）' : text, chars: text.length })
    } else if (ev.type === 'assistant/message') {
      const text = textOfMessage(ev.data && ev.data.message)
      if (full) {
        // 全文模式：工作 AI 回复按原文带上（受单条上限与总预算约束），但标注为"参考上下文"
        seq.push({ role: 'work-ai-full', text: text.length > HISTORY_ASSISTANT_MAX_CHARS ? text.slice(0, HISTORY_ASSISTANT_MAX_CHARS) + '…（截断）' : text, chars: text.length, tools: 0 })
      } else {
        // 回合模式：工作 AI 的**内容不进入上下文**，只留中性元数据 —— 防止把它的方案/口吻当成用户意图
        seq.push({ role: 'work-ai', chars: text.length, tools: 0 })
      }
    } else if (ev.type === 'tool/call') {
      const last = seq[seq.length - 1]
      if (last && (last.role === 'work-ai' || last.role === 'work-ai-full')) last.tools += 1
    }
  }
  ...
  return { mode: m, turns: n, full, degraded: false, userTurns, items, chars: total, omittedTurns, budget }
}
...
const STATE_FILE = join(DSH_HOME, 'prompt-toolkit.json')
/** 改名前叫 prompt-optimizer.json。首次运行做一次性搬迁，别让档位/权限设置丢了（旧文件不删）。 */
const LEGACY_STATE_FILE = join(DSH_HOME, 'prompt-optimizer.json')

/**
 * 一次性搬迁：新文件还不存在、老文件在 → 原样接过来。
 * 改名只碰"文件名"，不碰内容格式，所以直接复制即可。
 */
function migrateOnce(newFile, oldFile) {
  try {
    if (existsSync(newFile) || !existsSync(oldFile)) return false
...
function loadPluginState() {
  migrateOnce(STATE_FILE, LEGACY_STATE_FILE)
...
function initSettingsNamespace(ctx) {
  try {
    ...
    settingsScope = settings.register('prompt-toolkit', schema)
...
/* ══════════════ 提示词收藏夹 ══════════════
   落盘在**独立文件**里，和插件状态分开：状态每次改档位/权限都会重写，
   收藏是"用户资产"，混在一起容易被高频写入牵连，也不方便单独备份。
   全部读写都在本机，不经过任何网络。 */
const FAV_FILE = join(DSH_HOME, 'prompt-toolkit-favorites.json')
/** 改名前叫这个；同样一次性搬迁。 */
const LEGACY_FAV_FILE = join(DSH_HOME, 'prompt-optimizer-favorites.json')
const FAV_CAP = 200          // 条数上限，超出后丢最早的（置顶的不丢）
const FAV_TITLE_MAX = 80
const FAV_TEXT_MAX = 20000

/** 取正文首行做默认标题（都是空行时退化成前 40 字）。 */
function favoriteTitleOf(text) {
  const lines = String(text || '').split('\n').map((s) => s.trim()).filter((s) => s.length > 0)
  return (lines[0] || String(text || '').trim()).slice(0, 40)
}

/** 单条收藏的清洗：没有正文就丢弃；id/时间缺失则补上。 */
function sanitizeFavorite(raw) {
  ...
  const now = Date.now()
  const out = {
    id: typeof raw.id === 'string' && raw.id ? raw.id.slice(0, 64) : 'fav_' + now.toString(36) + Math.random().toString(36).slice(2, 6),
    text: text.slice(0, FAV_TEXT_MAX),
    title: (typeof raw.title === 'string' && raw.title.trim() ? raw.title.trim() : favoriteTitleOf(text)).slice(0, FAV_TITLE_MAX),
    pinned: raw.pinned === true,
    createdAt: Number.isFinite(raw.createdAt) ? Math.round(raw.createdAt) : now,
    updatedAt: Number.isFinite(raw.updatedAt) ? Math.round(raw.updatedAt) : now,
  }
  if (typeof raw.tier === 'string' && TIER_IDS.has(raw.tier)) out.tier = raw.tier
  if (typeof raw.source === 'string' && raw.source) out.source = raw.source.slice(0, 32)
  return out
}

/** 读收藏：按 createdAt 倒序返回；置顶只影响展示顺序，由客户端排。 */
function loadFavorites() {
  migrateOnce(FAV_FILE, LEGACY_FAV_FILE)
  const raw = readJson(FAV_FILE, null)
  const list = raw && Array.isArray(raw.items) ? raw.items : (Array.isArray(raw) ? raw : [])
  const out = []
  const seen = new Set()
  for (const it of list) {
    const f = sanitizeFavorite(it)
    if (!f || seen.has(f.id)) continue
    seen.add(f.id)
    out.push(f)
  }
  out.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))
  return out.slice(0, FAV_CAP)
}

/** 写收藏：超出上限时丢最早的，但**绝不丢置顶的**。 */
function saveFavorites(items) {
  let list = (Array.isArray(items) ? items : []).map(sanitizeFavorite).filter(Boolean)
  list.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))
  if (list.length > FAV_CAP) {
    const pinned = list.filter((x) => x.pinned)
    const rest = list.filter((x) => !x.pinned)
    list = pinned.concat(rest.slice(0, Math.max(0, FAV_CAP - pinned.length)))
    list.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))
  }
  try { writeFileSync(FAV_FILE, JSON.stringify({ version: 1, updatedAt: Date.now(), items: list }, null, 2)) } catch { /* best effort */ }
  return list
}

/** 收藏夹的一个动作；返回 { code, body }，由路由统一 send。 */
function favoritesAction(patch) {
  const action = String((patch && patch.action) || '')
  let items = loadFavorites()
  const now = Date.now()
  if (action === 'add') {
    const f = sanitizeFavorite({ id: patch.id, text: patch.text, title: patch.title, tier: patch.tier, source: patch.source, createdAt: now, updatedAt: now })
    if (!f) return { code: 400, body: { ok: false, error: 'empty-text' } }
    const dup = items.find((x) => x.text === f.text)
    if (dup) return { code: 200, body: { ok: true, deduped: true, item: dup, items } }
    items.unshift(f)
    items = saveFavorites(items)
    return { code: 200, body: { ok: true, item: f, items } }
  }
  if (action === 'update') {
    const id = String(patch.id || '')
    const idx = items.findIndex((x) => x.id === id)
    if (idx < 0) return { code: 404, body: { ok: false, error: 'not-found' } }
    const cur = items[idx]
    // 先更新正文再定标题：标题留空时的兜底要按**新的**正文取首行，否则会拿旧正文算
    if (typeof patch.text === 'string' && patch.text.trim()) cur.text = patch.text.trim().slice(0, FAV_TEXT_MAX)
    if (typeof patch.title === 'string') cur.title = (patch.title.trim() || favoriteTitleOf(cur.text)).slice(0, FAV_TITLE_MAX)
    if (typeof patch.pinned === 'boolean') cur.pinned = patch.pinned
    cur.updatedAt = now
    items = saveFavorites(items)
    return { code: 200, body: { ok: true, item: cur, items } }
  }
  if (action === 'remove') {
    const id = String(patch.id || '')
    const before = items.length
    items = saveFavorites(items.filter((x) => x.id !== id))
    return { code: 200, body: { ok: true, removed: before - items.length, items } }
  }
  if (action === 'clear') {
    items = saveFavorites([])
    return { code: 200, body: { ok: true, cleared: true, items } }
  }
  if (action === 'import') {
    const incoming = Array.isArray(patch.items) ? patch.items : []
    const byId = new Set(items.map((x) => x.id))
    const byText = new Set(items.map((x) => x.text))
    let added = 0
    let skipped = 0
    for (const raw of incoming) {
      const f = sanitizeFavorite(raw)
      if (!f || byId.has(f.id) || byText.has(f.text)) { skipped += 1; continue }
      byId.add(f.id)
      byText.add(f.text)
      items.push(f)
      added += 1
    ...
    items = saveFavorites(items)
    return { code: 200, body: { ok: true, added, skipped, items } }
  }
...
async function executeLiveRun(ctx, run) {
  ...
  try {
    ...
    if (digest) {
      run.history = { mode: digest.mode || run.historyMode || 'turns', turns: digest.turns, userTurns: digest.userTurns || 0, chars: digest.chars || 0, omittedTurns: digest.omittedTurns || 0, budget: digest.budget || null, degraded: digest.degraded === true, reason: digest.reason || null }
...
const PKG_NAME = '@dsh-external/dsh-prompt-toolkit'
const resolveTrail = []
...
const ROOT = findRoot(START_URL)
  || join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'plugins', 'dsh-prompt-toolkit')
...
const API_PATH = '/prompt-toolkit/api'
...
export function apply(ctx) {
  ...
  }, 'prompt-toolkit: watchdog + tier trigger')
  ...
  ctx.effect(() => {
    ...
    return () => { if (typeof off === 'function') off() }
  }, 'prompt-toolkit: host user/message recorder')
    ...
    handler: async (req, res) => {
      ...
      try {
        if (url.includes('/cmd')) {
          ...
          return send(200, {
            ok: true,
            host: '@dsh-external/dsh-prompt-toolkit',
            ts: Date.now(),
            cmd,
            cmdAgeMs: ageMs,
            cmdTtlMs: CMD_TTL_MS,
        ...
        if (url.includes('/stream')) {
          ...
          /* 快照里也带上 history：history 事件是在开跑最开始 publish 的，而客户端要等 /run
             返回之后才建 EventSource —— 只发一次事件的话它多半已经错过（上游的竞态）。
             放进初始快照后，早订/晚订都能拿到这次究竟读入了多少上下文。 */
          res.write('data: ' + JSON.stringify({ type: 'snapshot', status: run.status, reasoning: run.reasoning, text: run.text, error: run.error, firstDeltaMs: run.firstDeltaMs, usage: run.usage || null, history: run.history || null }) + '\n\n')
        ...
        if (url.includes('/favorites')) {
          // 收藏夹：GET 读列表；POST 带 action 做增删改 / 导入 / 清空
          if (req.method === 'POST') {
            const body = await readBody(req)
            let patch = {}
            try { patch = JSON.parse(body || '{}') } catch { patch = {} }
            const out = favoritesAction(patch)
            return send(out.code, out.body)
          }
          return send(200, { ok: true, items: loadFavorites(), file: FAV_FILE, cap: FAV_CAP })
        }
  ...
  }), 'prompt-toolkit: selftest api')

  ctx.logger?.info?.('[dsh-prompt-toolkit] host ready @ ' + API_PATH + ' (evidence: ' + EVIDENCE + ')')
}
