window.__ModuleLoader__.load({
	id: "@dsh-external/dsh-prompt-toolkit",
	factory: (require) => {
		var module = { exports: {} };
		...
		const API = "/prompt-toolkit/api";
		const NS = "dsh-prompt-toolkit";
		...
		*/
		...
		/* ⚠️ 让位：本仓库另一个插件 @dsh-external/dsh-prompt-favorites 是同一套收藏夹的独立版本，
		   它启动时会挂 window.__DPF_PRESENT__ = true。只要它在，本插件就把自己的收藏夹**整套收起来**
		   （菜单入口、动作栏按钮、对话里的书签注入）——两边单独装都完整，一起装也不重复。
		   读的是实时标记而不是启动时快照：插件加载顺序不保证，晚到的那个也能让先到的让位。 */
		function favoritesDeferred() {
			try { return window.__DPF_PRESENT__ === true; } catch (e) { return false; }
		}

		/* ══════════ 提示词收藏夹 ══════════
		   宿主把它落盘在 ~/.dsh/prompt-toolkit-favorites.json（独立文件，全部本机读写）。
		...
		function exportFavorites() {
			...
			try {
				...
				a.href = url;
				a.download = "prompt-toolkit-favorites-" + new Date().toISOString().slice(0, 10) + ".json";
		...
		function ensureMsgFavButtons() {
			if (favoritesDeferred()) {
				// 让位：已经插进去的按钮也撤掉，免得两枚书签并排
				try {
					for (const el of document.querySelectorAll("[" + MSGFAV_FLAG + "]")) el.remove();
				} catch (e) { /* noop */ }
				return 0;
			}
		...
		*/
		...
		function FavoritesPane() {
			...
			if (favoritesDeferred()) {
				// 独立收藏夹插件在场：本插件的这套已经让位，别让用户以为是空收藏夹
				return h("div", { className: "dpo-pane dpo-fav-pane", "data-dpo": "fav-pane", "data-deferred": "true" },
					h("div", { className: "dpo-pane-empty" },
						"收藏夹由独立插件 dsh-prompt-favorites 提供。",
						h("div", { className: "dpo-pane-empty-hint" }, "本插件已让位，请使用那个插件的「收藏夹」入口。")));
			}
			const all = Array.isArray(store.favorites) ? store.favorites : [];
		...
		*/
		...
		function dockFooter() {
			...
			return h("div", { className: "dpo-dock-foot", "data-dpo": "dock-foot" },
				h("div", { className: "dpo-dock-foot-side" },
					...
					// 收藏夹那两枚按钮：独立收藏夹插件在场时整体让位（见 favoritesDeferred）
					favoritesDeferred() ? null : h("button", {
		...
		const SIDEBAR_KIND = "prompt-toolkit";
		/** 收藏夹是右侧栏里的第二个 tab（与优化面板并排）。 */
		const FAV_KIND = "prompt-toolkit-favorites";
