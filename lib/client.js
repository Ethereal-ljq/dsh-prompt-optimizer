	...
	factory: (require) => {
		...
		*/
		function favoritesPanel(extra) {
			...
			/* 居中的对话框，而不是挂在按钮旁边的小弹层：
			   收藏夹要放列表 + 搜索 + 管理，本身就不是"一条小提示"；
			   而且它的入口有两个（胶囊菜单 / 拦截卡动作栏），分别在屏幕上下两头 ——
			   跟挂其中一边，从另一边进来就会显得跑到别处去了。居中对两边都一样。 */
			return h("div", {
				className: "dpo-fav-scrim", "data-dpo": "fav-scrim",
				onMouseDown: (e) => { if (e.target === e.currentTarget) closeFavorites(); },
			},
			h("div", Object.assign({
				className: "dpo-fav-panel", "data-dpo": "fav-panel",
				role: "dialog", "aria-modal": "true", "aria-label": "提示词收藏夹",
			}, extra || {}),
				h("div", { className: "dpo-fav-bar" },
					...
					h("button", {
						type: "button", className: "dpo-fav-close", "data-dpo": "fav-close", title: "关闭（Esc）",
						"aria-label": "关闭收藏夹",
						onClick: () => closeFavorites(),
					}, h(UI.IconCloseOutline16, { size: 16 })),
				),
				h("input", {
		...
		*/
		...
		function OptimizeChip() {
			...
			/* 收藏夹是居中对话框：不锚定任何元素，所以不参与上面的定位/外部点击逻辑，
			   只需要一条 Esc 关闭。面板里输入框的 Esc 会自己 stopPropagation（清空搜索词），
			   所以不会误关整个对话框。 */
			React.useEffect(() => {
				if (!favOpen) return undefined;
				const onKey = (e) => { if (e.key === "Escape") { e.preventDefault(); closeFavorites(); } };
			...
			return h(React.Fragment, null,
				...
				helpOpen
					...
					: null,
				/* 收藏夹：居中的对话框（不需要定位，样式交给 CSS 的遮罩 + 居中） */
				favOpen ? ReactDOM.createPortal(favoritesPanel({}), document.body) : null,
			);
		}
		...
		exports.apply = function apply(ctx) {
			...
			own(() => {
				...
				style.textContent = [
					...
					/* ══════════ 提示词收藏夹：居中的对话框 ══════════ */
					".dpo-fav-scrim{position:fixed;inset:0;z-index:1200;display:flex;align-items:center;justify-content:center;padding:24px;background:rgba(0,0,0,.45)}",
					".dpo-fav-panel{display:flex;flex-direction:column;box-sizing:border-box;width:470px;max-width:100%;max-height:min(74vh,580px);overflow:hidden;padding:0;border:1px solid var(--dsw-alias-border-l1,#3a3a3a);border-radius:14px;background:var(--dsw-specific-menu,var(--dsw-specific-tip,#1b1b1b));box-shadow:0 18px 48px rgba(0,0,0,.45);color:var(--dsw-alias-label-primary,#eee);font-size:12px;line-height:1.6}",
