window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const store = {
			...
			/* 模型菜单里哪些 provider 是展开的（按 provider id）。默认全折叠，
			   只在打开菜单时把"当前选中模型所在的那一家"展开。 */
			modelExpanded: {},
		...
		const CONTEXT_TURNS_PRESETS = [1, 2, 3, 5, 10];
		...
		/** 纵行菜单：档位 / 权限 / 模型 三组 + 帮助入口，一行一项、自上而下。 */
		function chipMenuItems() {
			...
			/* 每家 provider 收成一行（官方箭头图标 + 右侧说明），点了才展开它的模型。
			   默认全折叠，避免装了四五家 provider 之后菜单拉得极长；
			   右侧说明在"当前选中就在这一家"时会直接写出模型名，收起状态下也一眼看得出选的是谁。 */
			for (const g of groups) {
				const models = g.models || [];
				const expanded = Boolean(store.modelExpanded && store.modelExpanded[g.id] === true);
				...
				items.push({
					...
					icon: h(expanded ? UI.IconChevronDownOutline14 : UI.IconChevronRightOutline14, { size: 14 }),
					label: menuRow(g.name, hint),
				});
				if (!expanded) continue;
				for (const m of models) {
					const cur = cat && cat.current && cat.current.provider === g.id && cat.current.model === m.id;
					// 缩进一格，让"这些模型属于上面那家 provider"一眼看得出来
					items.push({ id: MODEL_PREFIX + g.id + "/" + m.id, label: menuRow(h("span", { className: "dpo-leaf" }, m.name), cur ? "会话当前" : "") });
				}
			}
			items.push({ type: "separator", id: "sep-ctx" });
			items.push({ type: "label", id: "lbl-ctx", text: "上下文（优化时读入）" });
			items.push({ id: CONTEXT_PREFIX + "off", label: menuRow("不读取", "只优化你这一句") });
			for (const n of CONTEXT_TURNS_PRESETS) {
				items.push({ id: CONTEXT_PREFIX + "turns:" + n, label: menuRow("最近 " + n + " 回合", n === 3 ? "推荐" : "") });
			}
			items.push({ id: CONTEXT_PREFIX + "full", label: menuRow("全文", "与工作 AI 看到的上下文一致") });
		...
		function chipSelectedIds() {
			...
			// 上下文：全文 / 不读取 / 某个预设回合数（不在预设上时就不打勾，避免误导）
			if (store.historyMode === "full" && store.fullOn === true) ids.push(CONTEXT_PREFIX + "full");
			else if (!store.turns) ids.push(CONTEXT_PREFIX + "off");
			else if (CONTEXT_TURNS_PRESETS.indexOf(store.turns) >= 0) ids.push(CONTEXT_PREFIX + "turns:" + store.turns);
			return ids;
		}
		...
		function onChipSelect(id) {
			...
			// 上下文：三选一（不读取 / 最近 N 回合 / 全文）
			if (id.indexOf(CONTEXT_PREFIX) === 0) {
				const v = id.slice(CONTEXT_PREFIX.length);
				if (v === "off") { store.turns = 0; store.historyMode = "turns"; store.fullOn = false; }
				else if (v === "full") { store.historyMode = "full"; store.fullOn = true; store.turns = TURNS_MAX; }
				else if (v.indexOf("turns:") === 0) {
					store.turns = Math.max(0, Math.min(TURNS_MAX, Number(v.slice(6)) || 0));
					store.historyMode = "turns"; store.fullOn = false;
				}
				store.touched = true;
				beacon("context-change", { turns: store.turns, historyMode: store.historyMode, fullOn: store.fullOn });
				persistState();
				emit();
				return;
			}
			// provider 行是折叠开关，不是"选中" —— 只切展开态，不改任何设置
			if (id.indexOf(PROVIDER_PREFIX) === 0) {
				const pid = id.slice(PROVIDER_PREFIX.length);
				const next = Object.assign({}, store.modelExpanded || {});
				if (next[pid] === true) delete next[pid]; else next[pid] = true;
				store.modelExpanded = next;
				emit();
				return;
			}
		...
		*/
		...
		function OptimizeChip() {
			...
			const menuKey = open
				? Object.keys(store.modelExpanded || {}).sort().join(",") + "#" +
			...
			const [helpMax, setHelpMax] = React.useState(520);
			React.useLayoutEffect(() => {
				...
				const measure = () => {
					...
					setHelpMax(Math.max(180, Math.round(top - HELP_GAP - HELP_MARGIN)));
				};
				measure();
					...
					anchor: h("button", {
						...
						onClick: () => {
							...
							if (store.menuOpen) {
								...
								// 每次打开菜单都从"全折叠"开始，只把当前选中模型所在的那一家展开 ——
								// 这样既不会看到一长串模型，也不会出现"当前选的是谁"看不到的情况。
								const next = {};
								if (store.modelSel && store.modelSel.provider) next[store.modelSel.provider] = true;
								store.modelExpanded = next;
							}
							emit();
						},
		...
		exports.apply = function apply(ctx) {
			...
			own(() => {
				...
				style.textContent = [
					...
					/* 展开后的模型行：缩进一格，标明它属于上面那家 provider */
					".dpo-leaf{display:inline-block;padding-left:16px}",
