	...
	factory: (require) => {
		...
		*/
		...
		function favoriteCurrentOutput() {
			...
			if (!body) { showNotice("现在还没有可收藏的产出"); return; }
			beacon("favorite-add", { chars: body.length, tier: run ? run.tier : store.tier });
			favoritesPost({ action: "add", text: body, tier: run ? run.tier : store.tier, source: "optimized" }).then((d) => {
		...
		function favoritesPanel(extra) {
			...
			const rows = shown.map((it) => {
				...
				if (store.favEditId === it.id) {
					const commit = () => { const v = store.favEditText; favoritesPost({ action: "update", id: it.id, title: v }).then(() => { store.favEditId = null; emit(); }); };
					return h("div", { key: it.id, className: "dpo-fav-row", "data-dpo": "fav-row", "data-id": it.id, "data-editing": "true" },
						h("input", {
							className: "dpo-fav-input", "data-dpo": "fav-rename-input", type: "text",
							value: store.favEditText || "", spellCheck: false,
							onChange: (e) => { store.favEditText = e.target.value; emit(); },
							onKeyDown: (e) => {
								e.stopPropagation();
								if (e.key === "Enter") { e.preventDefault(); commit(); }
								else if (e.key === "Escape") { e.preventDefault(); store.favEditId = null; emit(); }
							},
						}),
						h("button", { type: "button", className: "dpo-fav-btn", "data-dpo": "fav-rename-save", onClick: commit }, "保存"),
						h("button", { type: "button", className: "dpo-fav-btn", onClick: () => { store.favEditId = null; emit(); } }, "取消"),
					);
				}
				return h("div", { key: it.id, className: "dpo-fav-row", "data-dpo": "fav-row", "data-id": it.id, "data-pinned": String(it.pinned === true) },
					...
					confirming
						...
						: h("div", { className: "dpo-fav-acts" },
							...
							h("button", { type: "button", className: "dpo-fav-btn", "data-dpo": "fav-rename", title: "重命名", onClick: () => { store.favEditId = it.id; store.favEditText = it.title || ""; store.favConfirm = null; emit(); } }, "改名"),
			...
			return h("div", Object.assign({ className: "dpo-fav-panel", "data-dpo": "fav-panel", role: "dialog" }, extra || {}),
				h("div", { className: "dpo-help-bar" },
					h("button", {
						type: "button", className: "dpo-back", "data-dpo": "fav-back", title: "返回设置菜单",
						onClick: () => { store.favOpen = false; store.menuOpen = true; emit(); },
					},
						h(UI.IconChevronLeftOutline14, { size: 14 }),
						h("span", null, "返回"),
					),
					h("span", { className: "dpo-help-head" }, "提示词收藏夹"),
		...
		*/
		...
		function OptimizeChip() {
			...
			const favPanelRef = React.useRef(null);
			...
			const favOpen = store.favOpen === true;
			...
			React.useLayoutEffect(() => {
				if (!helpOpen && !favOpen) return undefined;
				const measure = () => {
			...
			const favPos = UI.useAnchoredPosition({ open: favOpen, anchorRef: anchorRef, panelRef: favPanelRef, side: "top", gap: HELP_GAP, margin: HELP_MARGIN });
			UI.useDismissOnOutsidePointer(anchorRef, helpOpen, () => { if (store.helpOpen) { store.helpOpen = false; emit(); } }, panelRef);
			UI.useDismissOnOutsidePointer(anchorRef, favOpen, () => { if (store.favOpen) closeFavorites(); }, favPanelRef);
			...
			return h(React.Fragment, null,
				h(UI.Menu, {
					...
					anchor: h("button", {
						...
						onClick: () => {
							...
							if (store.menuOpen) { store.helpOpen = false; store.favOpen = false; }
							emit();
						},
					},
				...
				helpOpen
					...
					: null,
				/* 收藏夹面板：同一套"先渲染、后定位"的做法 */
				favOpen
					? ReactDOM.createPortal(favoritesPanel({
						ref: favPanelRef,
						style: {
							left: (favPos ? favPos.left : 0) + "px",
							top: (favPos ? favPos.top : 0) + "px",
							maxHeight: panelMax + "px",
							visibility: favPos ? "visible" : "hidden",
						},
					}), document.body)
					: null,
			);
		...
		function dockHeader() {
			...
			/* 头部拆成「折叠开关 + 收藏夹入口」两件事：
			   折叠开关占满剩余宽度，收藏夹单独一枚小按钮 —— 免得为了加个入口
			   把动作栏挤到五六个按钮。 */
			return h("div", { className: "dpo-dock-headrow" },
			h("button", {
			...
			h("button", {
				type: "button", className: "dpo-dock-favbtn", "data-dpo": "dock-fav-open",
				title: "提示词收藏夹（存下的优化产出，点一条可填回输入框）",
		...
		const DockHost = () => h(DockBoundary, null, h(OptimizeDock));

		/**
		* 拦截卡：坐在输入框上方（conversation.input.dock），形态取自 DSH 自己的
		* TodoPanel 折叠卡与审批卡动作条 —— 它是一个正常的、占位的、跟着页面走的面板，
		* 没有拖拽、没有缩放手柄、没有位置落盘，也不会盖住任何内容。
		* 该槽不区分 hero/composer，所以新对话页与会话里表现完全一致。
		*/
		function OptimizeDock() {
			...
			React.useEffect(() => {
				store.dockMounted = true;
				return () => { store.dockMounted = false; };
			}, []);
			const o = store.overlay;
			const run = store.run;
			const expanded = dockExpanded();
			// 产出编辑框自动增高（上限交给 CSS 的 max-height，不再固定高度套小滚动条）
			const text = (store.reviewText !== undefined && store.reviewText !== null) ? store.reviewText : (run ? run.text : "");
			React.useLayoutEffect(() => {
				const el = outRef.current;
				if (!el) return;
				el.style.height = "auto";
				el.style.height = Math.min(el.scrollHeight, Math.round((window.innerHeight || 800) * 0.45)) + "px";
			}, [text, expanded, o.open]);
			if (o.open !== true) return null;
			// 会话隔离：卡片只属于触发它的那个会话
			if (o.sessionId !== undefined && o.sessionId !== null && store.viewSessionId !== null && o.sessionId !== store.viewSessionId) return null;
			if (!isActiveInstance()) return null; // 旧实例不再渲染（否则"后台在跑、卡片不显示"）
			store.renderCount = (store.renderCount || 0) + 1;
			const st = dockState();
			return h("div", {
				className: "dpo-dock", "data-dpo": "dock",
				"data-state": st, "data-open": String(expanded),
			},
				dockHeader(),
				expanded ? dockBody(outRef) : null,
				dockFooter(),
			);
		}
		...
		exports.apply = function apply(ctx) {
			...
			own(() => {
				...
				style.textContent = [
					...
					/* ══════════ 拦截卡头部：折叠开关 + 收藏夹入口（拆成两件事，免得动作栏被挤满） ══════════ */
					".dpo-dock-headrow{display:flex;align-items:center;min-width:0}",
					".dpo-dock-head{flex:1 1 auto;min-width:0;width:auto}",
					".dpo-dock-favbtn{flex:none;height:22px;margin-right:8px;padding:0 8px;border-radius:6px;border:1px solid transparent;background:0 0;color:var(--dsw-alias-label-tertiary,#999);font-family:inherit;font-size:11px;line-height:20px;white-space:nowrap;cursor:pointer;transition:background .16s,color .16s}",
					".dpo-dock-favbtn:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(127,127,127,.14));color:var(--dsw-alias-label-primary,#eee)}",

					/* ══════════ 提示词收藏夹面板：与帮助面板同一个外壳 ══════════ */
					".dpo-fav-panel{position:fixed;z-index:1100;box-sizing:border-box;width:420px;overflow-y:auto;padding:14px 16px;border:0;border-radius:20px;background:var(--dsw-specific-menu,var(--dsw-specific-tip,#1b1b1b));--dsw-elevation-stroke-color:var(--dsw-alias-border-l1);box-shadow:var(--dsw-elevation-prominent,0 6px 24px rgba(0,0,0,.35));color:var(--dsw-alias-label-primary,#eee);font-size:12px;line-height:1.6}",
					".dpo-fav-count{margin-left:auto;flex:none;color:var(--dsw-alias-label-caption,#888);font-size:11px}",
					".dpo-fav-search{box-sizing:border-box;width:100%;height:28px;margin-bottom:8px;padding:0 10px;border-radius:8px;border:1px solid var(--dsw-alias-border-l1,#3a3a3a);background:0 0;color:var(--dsw-alias-label-primary,#eee);font-family:inherit;font-size:12px}",
					...
					".dpo-fav-list{display:flex;flex-direction:column;gap:2px;margin-bottom:8px}",
					...
					".dpo-fav-input{box-sizing:border-box;flex:1 1 auto;min-width:0;height:26px;padding:0 8px;border-radius:7px;border:1px solid var(--dsw-alias-border-l1,#3a3a3a);background:0 0;color:var(--dsw-alias-label-primary,#eee);font-family:inherit;font-size:12px}",
					...
					".dpo-fav-empty{padding:14px 4px;color:var(--dsw-alias-label-caption,#888);font-size:12px;text-align:center}",
					".dpo-fav-foot{display:flex;align-items:center;gap:6px;padding-top:10px;border-top:1px solid var(--dsw-alias-border-l2,var(--dsw-alias-border-l1,#333))}",
			...
			/* 拦截卡：挂在"输入框上方的那条 dock"（conversation.input.dock）—— 与官方 TodoPanel、
			   队列面板同一个位置、同一种折叠卡，而且是布局流里的正常占位（不浮、不盖内容）。
			   选它而不是 conversation.composer.dock 的原因：后者带 variant==="composer" 的条件，
			   新对话页（variant==="hero"）根本不渲染 —— 而"新对话里按下回车被拦截"恰恰是最需要它的场景。
			   注册必须幂等（list 槽同 id 同 priority 会抛错）。 */
			const ensureDockRegistration = () => {
				if (store.dockRegistered === true) return;
				store.dockRegistered = true;
				own(() => {
					const dispose = ctx.slots.inject("conversation.input.dock", () => ctx.slots.register({
						name: "conversation.input.dock",
						id: "prompt-optimizer",
						order: 30,
					}, DockHost));
					return () => {
						store.dockRegistered = false;
						if (typeof dispose === "function") dispose();
					};
				}, NS + ": interception dock");
			};
			ensureDockRegistration();

			// 卡片自愈：store 说"开着"但 DOM 里没有节点 → 重挂一次（防"卡片无声消失"）；挂了就停
			own(() => {
				let tries = 0;
				const timer = window.setInterval(() => {
					tries += 1;
					if (!store.overlay.open) { if (tries > 1) window.clearInterval(timer); return; }
					if (store.dockMounted === true || document.querySelector('[data-dpo="dock"]')) {
						if (tries > 1) { beacon("overlay-healed", { tries }); window.clearInterval(timer); }
						return;
					}
					if (tries === 1) beacon("remount-overlay", { tries, runStatus: store.run ? store.run.status : null, tier: store.tier });
					ensureDockRegistration();
